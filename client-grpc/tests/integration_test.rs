...
fn get_log_string(function: &str, name: &str) -> String {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "stub_backends")] {
            format!("({}) (MOCK) {} server.", function, name)
        } else {
            format!("({}) {} client.", function, name)
        }
    }
}
...
async fn test_client_requests_and_logs() {
    ...
    use svc_devops_test_client_grpc::prelude::*;

    let name = "devops_test";
    let (server_host, server_port) =
        lib_common::grpc::get_endpoint_from_env("SERVER_HOSTNAME", "SERVER_PORT_GRPC");

    let client = DevopsTestClient::new_client(&server_host, server_port, name);
    ...
    {
        let result = client.is_ready(devops_test::ReadyRequest {}).await;
        println!("{:?}", result);
        assert!(result.is_ok());
